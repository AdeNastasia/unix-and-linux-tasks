...
### Ответ 6
Я так поняла, ято нужна такая схема. Или нет?
![alt text](boot.png)
...
а) первая запись разрешает пользователям matt и lisa конфигурировать принтер, очищать его очередь и перезапускать демоны печати на компьютере
printserver
